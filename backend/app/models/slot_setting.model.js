...
slotSettingsSchema.index({ created_at: 1 });
slotSettingsSchema.index({ updated_at: 1 });

slotSettingsSchema.index({
    live_stream: 1,
    name: 1,
});

const model = omniflixStudio.model('slot_setting', slotSettingsSchema)
    .on('index', (error) => {
        if (error) {
            logger.error(error);
        }
